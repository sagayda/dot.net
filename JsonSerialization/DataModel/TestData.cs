...
public static class TestData
{
	...
	public static ApartmentHouse GetApartmentHouse(int apartmentsCount)
	{
		...
		house.Apartments.AddRange(GetApartments(address, apartmentsCount));
	...
	public static (IEnumerable<Building> Buildings, IEnumerable<Tenant> Tenants) GetTestData(float volume)
	{
		...
		var addresses = apartmentHouses.SelectMany(apartmentHouse => apartmentHouse.Apartments.Select(apartment => apartment.Address as Address)).Concat(privateHouses.Select(house => house.Address)).OrderBy(address => _random.Next(0, 1024));

		Tenant currentTenant = GetTenant();
		foreach (var address in addresses)
		{
			currentTenant.Registrations.Add(new(new DateTime(_random.Next(2000, 2024), _random.Next(1, 13), _random.Next(1, 29)), address));
